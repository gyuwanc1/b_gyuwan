...
console.log(i++); //9 :변수 i의값을 먼저 도출 후 연산 1을 더한다.
console.log(i++); //10
console.log(i); //11
console.log(++i); //12 :변수 i의 값을 먼저 연산후에 도출
console.log(++i); //13
console.log(i); //11

//위 전치/ 후치의 계산차이는 실무에서느 큰차이는 없다
//우리의 일상과 비교한다면, 한국식 나이계싼과 미국신 나이계산 차이
//실무작업에서는 정확한 값을 도출 후 작업을 진행하는 것을 권장
...
var compare = n === i; // === n값과 i의값이  같지 않으므로 false
...
// 한쪽이 완전한 충족을 하면 다른쪽은 비교하지 않는다!
// 단, 하나의 비교연산자 ( |,&)사용시 결과는 0,1로 도출
// 팁: 조금이라도 빠른 연산을 위해
// ||은 true가 나오기 쉽거나 우선이 되는 내용을 
// &&는 false가 나오기 쉽더나 우선이 되는 내용을 좌변에 배치

var compare2 = true || false;  // ||둘중 하나이상 참이면 true
console.log( compare2);

compare2 = true && false; // && 둘중하나라도 거짓이면 false
console.log( compare2) ;
...
//----------------------------------------------------------------------
//1.당신이 개발자라면 프론트와 밴엔드 분야중 어느쪽인가?
//2.프론트엔드 분야에서 익혀야 하는 언어/기술 [];
//3.백엔드 분야에서 익혀야 하는 언어/기술
console.clear();

//1.----------
var listD = []

var developT = function () {
  listD= [ 'frontend', 'css'];
  return listD;
};

var developF = function () {
  listD = ['backend'];
  return listD;
};

// var program = 'front';
// (program === 'front')? developT() : developF() ;
// console.log(listD); 
//----------------------------------------
...
var programer = function (field){
  ...
  // (data)? developT() : developF() ;

  (data)? 
    developT():
    (data2)? developF() : listD=[];
};
programer ('back');
console.log (listD);