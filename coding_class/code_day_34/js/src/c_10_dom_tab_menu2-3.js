...
// 1. 카드 클릭시 모달창을 띄울 요소를 생성 (이전에는 html에서 존재, 클릭시 생성)
...
var fnCloseBtnAction = function(){
  ...
  elModalClose.addEventListener('click', function(e){
    ...
    elEventLiArr[OPTION_INDEX].children[0].focus();//몇번째를 클릭했는지
  });
};
...
var fnmakeModal = function(){
  ...
  _mkModal.className = 'event_modal' // classname
  _mkModal.innerHTML = MODAL_CODE;

  //생성된 요소 삽입
  elContentInner.after(mkModal);
  
  //내용이 배치되었으므로, 선택자에 관련 변수 설정 (전역에서 인지 할 수 있도록 전역변수 처리)
...
//닫기버튼 생성
//닫기클릭시 원래의 li번째에 focus

//=================================================
//배치----------------------------------------------
//선택자들
//선택자중 (모달은 현재 없으니 생성 후 처리)

//닫기버튼 처리 함수
//모달 생성 함수 (모달생성함수를 수행시, 닫기 버튼처리 함수를 동작하도록)
